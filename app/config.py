...
class Config:
    ...
    # An uploader may connect their own archive.org account and mirror a live
    # show into the Internet Archive as it is filed. Opt in per show; nothing
    # here can fail an upload.
    #
    # There is no OAuth to configure: the app either trades the uploader's
    # archive.org email and password for that account's S3 keys, or takes a
    # pair they generated by hand. Set ARCHIVE_ORG_PASSWORD_LOGIN=false to
    # offer only the second.
    archive_org_enabled: bool = field(
        default_factory=lambda: _env_bool("ARCHIVE_ORG_ENABLED", True)
    )
    archive_org_password_login: bool = field(
        default_factory=lambda: _env_bool("ARCHIVE_ORG_PASSWORD_LOGIN", True)
    )
    ...
    # Stored credentials are encrypted with a key derived from this. It falls
    # back to SESSION_SECRET, which is what a normal deployment already has;
    # behind an authenticating proxy there is no session to sign, so a
    # deployment that wants this feature sets it explicitly.
    archive_org_secret: str = field(
        default_factory=lambda: _env("ARCHIVE_ORG_SECRET") or _env("SESSION_SECRET")
    )
    ...
    def archive_org(self) -> bool:
        """Can an uploader connect an archive.org account?

        Enabled, and with somewhere safe to keep the keys. Without a secret to
        derive an encryption key from, the honest answer is to hide the
        feature rather than store somebody's credentials in the clear.
        """
        return self.archive_org_enabled and bool(self.archive_org_secret)
