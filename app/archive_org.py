...
Two halves:
  ...
  * `sign_in` / `verify` -- how an uploader connects their own archive.org
    account. Archive.org publishes no OAuth for third-party apps. What it has
    is `/services/xauthn/`, which trades an email and password for that
    account's S3-like keys (exactly what `ia configure` does), and keys the
    user can generate by hand at archive.org/account/s3.php. Both roads end at
    the same `Credentials`, and the password is never written down.

  * `Publisher` -- the upload, over the S3-like API: one PUT per file to
    https://s3.us.archive.org/<identifier>/<name>, with the item's metadata
    riding along as x-archive-meta-* headers on the first request.
"""
...
class ArchiveAuthError(Exception):
    """Connecting an account failed; the message is safe to show the user."""
...
def sign_in(
    ...
    *,
    ...
    """Trade an archive.org email and password for that account's S3 keys.

    The password goes to archive.org and nowhere else -- not to disk, not into
    a log line, not back to the browser. Only the keys it returns are kept.
    """
    if not email.strip() or not password:
        raise ArchiveAuthError("Enter your archive.org email and password.")
    try:
        with httpx.Client(timeout=timeout) as client:
            response = client.post(
                f"{archive_base}/services/xauthn/",
                params={"op": "login"},
                data={"email": email.strip(), "password": password},
            )
    except httpx.HTTPError as exc:
        raise ArchiveAuthError("Could not reach archive.org to sign in.") from exc

    try:
        payload = response.json() or {}
    except ValueError as exc:
        raise ArchiveAuthError("archive.org sent back something unreadable.") from exc

    if not payload.get("success"):
        raise ArchiveAuthError(_sign_in_message(payload))

    values = payload.get("values") or {}
    s3 = values.get("s3") or {}
    access, secret = str(s3.get("access") or ""), str(s3.get("secret") or "")
    if not access or not secret:
        # A successful sign-in with no keys means the account has never
        # generated a pair. Say so, rather than storing an empty credential
        # that fails confusingly on the first upload.
        raise ArchiveAuthError(
            "That account has no API keys yet. Generate a pair at "
            "archive.org/account/s3.php and paste them in instead."
        )
    return Credentials(
        access=access,
        secret=secret,
        screenname=str(values.get("screenname") or ""),
        username=str(values.get("itemname") or ""),
    )


def _sign_in_message(payload: dict[str, Any]) -> str:
    reason = str((payload.get("values") or {}).get("reason") or payload.get("error") or "")
    if reason == "account_not_found":
        return "No archive.org account with that email address."
    if reason == "account_bad_password":
        return "That password was not right."
    if "captcha" in reason.lower():
        # Archive.org can put a challenge in front of a password sign-in that
        # this app has no way to answer. Pasted keys sidestep it entirely.
        return (
            "archive.org wants a captcha for this sign-in. Generate keys at "
            "archive.org/account/s3.php and paste them in instead."
        )
    return f"archive.org refused the sign-in{f': {reason}' if reason else '.'}"
...
def verify(
    ...
    *,
    ...
    """Check a key pair and read back whose it is.

    Used for pasted keys, and again for keys from a sign-in: it is the only
    way to learn the screen name a pasted pair belongs to, and it means a
    typo is caught while someone is looking at the form rather than an hour
    later at the end of an upload.
    """
    access, secret = access.strip(), secret.strip()
    if not access or not secret:
        raise ArchiveAuthError("Enter both the access key and the secret key.")
    try:
        with httpx.Client(timeout=timeout) as client:
            response = client.get(
                f"{s3_endpoint}/",
        ...
        raise ArchiveAuthError("Could not reach archive.org to check those keys.") from exc

    if not payload.get("authorized"):
        raise ArchiveAuthError(
            str(payload.get("error") or "archive.org did not recognise those keys.")
        )
    return Credentials(
        access=access,
        secret=secret,
        screenname=str(payload.get("screenname") or ""),
        username=str(payload.get("username") or payload.get("itemname") or ""),
    )
